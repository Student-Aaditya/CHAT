...
def btech_router(query: str):
    ...
    branch_hits = [
    branch for branch, aliases in BRANCH_ALIASES.items()
    if any(re.search(rf"\b{re.escape(a)}\b", q) for a in aliases)
]

    if len(branch_hits) > 1:
        return "Please ask for one branch at a time. Example:\nPlacement record of Btech cse or For Specilization Btech cy or Btech ds"
    ...
    if "seat" in q:
        responses.append(f"Seats: {c['properties'].get('seats','NA')}")

    if "duration" in q or "year" in q:
        responses.append(f"Duration: {c['properties'].get('duration','NA')}")

    if "eligibility" in q:
        responses.append(f"Eligibility: {c['properties'].get('eligibility','NA')}")

    if "fee" in q:
        responses.append(f"Fees: {c['properties'].get('fees','Check admission dept')}")
